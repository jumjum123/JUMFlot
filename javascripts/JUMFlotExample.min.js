(function(e){function q(a){var c=e(this)[0].attributes,c=r(c.docuobj.nodeValue,c.prevname.nodeValue,c.actname.nodeValue);e.plot.JUMlib.library.showTooltip(a.pageX,a.pageY,c)}function s(a){e.ajax({url:a,method:"GET",dataType:"text",success:function(a){e("#placeholderSource").html(a)},error:function(a,e,b){console.log(a,e,b)}})}function n(a,c,g){c||(c=8);var b="";g?b+="<ul>\n":(g=0,b+='<ul id="sitemap" class="treeview">\n');if("object"===typeof a)for(var d in a)"object"===typeof a[d]?(b=e.isNumeric(d)?
b+("<b>["+d+"]</b>"):b+("<li><b>"+d+"</b>"),a[d].docu&&(b+="&nbsp;<dfn>"+a[d].docu+"</dfn>"),a[d].defVal&&"none"!==a[d].defVal&&(b+="&nbsp;(<code>"+a[d].defVal+"</code>)"),b+="</li>\n",c>g&&(b+=n(a[d],c,g+1))):"docu"!==d&&"defVal"!==d&&(b+="<li>"+a[d]+"</li>\n");return b+"</ul>\n"}function t(a,c,g,b,d,f){function h(a,d,e){var c="";0>d?(d=0,c+='<ul id="sitemap" class="treeview">\n'):c+="<ul>\n";if("object"===typeof a)for(var f in a)if("object"===typeof a[f])c+="<li><b>"+k(e,f)+"</b>&nbsp;<i>"+l(a[f])+
"</i></li>\n",g>d&&(c+=h(a[f],d+1,e+"."+f));else if("function"===typeof a[f])c=!0===b?c+("<li>"+k(e,f)+"&nbsp;"+a[f].toString()+"</li>\n"):c+("<li>"+k(e,f)+"&nbsp;<i>"+l(a[f])+"</i></li>\n");else{var n="<li>"+k(e,f)+"=",m=a[f];"string"===typeof m&&(m=m.replace(/</g,"&lt;"),m=m.replace(/>/g,"&gt;"));c+=n+m+"&nbsp;<i>"+l(a[f])+"</i></li>\n"}else c+="<li>"+a+"&nbsp;<i>"+l(a)+"</i></li>\n";return c+"</ul>\n"}function k(a,b){return'<span class="jumflothelp" docuobj="'+c+'" prevname="'+a+'" actname="'+
b+'">'+b+"</span>"}function l(a){var b;b=typeof a;e.isArray(a)&&(b+=" Array");return b}g||(g=6);b||(b=!1);return h(a,-1,"")}function u(a){a=JSON.stringify(a);a=a.replace(/{/g,'{<blockquote style="padding:1px; margin-top:1px; margin-bottom:1px">');a=a.replace(/}/g,"</blockquote>}");a=a.replace(/\[/g,'[<blockquote style="padding:1px; margin-top:1px; margin-bottom:1px">');return a=a.replace(/\]/g,"</blockquote>]")}function r(a,c,e){var b="";a="$.plot.JUMExample.docu."+a+c+"."+e;try{eval(a+".docu")&&
(b=eval(a+".docu")),eval(a+".Default")&&(b+="Default:"+eval(a+".Default"))}catch(d){b+="nothing found, please see flot API"}return a+"<hr>"+b}var v=null,w=null,p=null;e.plot.JUMExample={};e.plot.JUMExample.init=function(a,c,g){var b="",d;for(d=0;d<c.length;d++)b+="<h3>"+c[d].header+"</h3>",b+=u(c[d].data);e("#placeholderData").html(b);b="";for(d=0;d<g.length;d++)b+="<h3>"+g[d].header+"</h3>",b+=t(g[d].data,a+"."+a+".options");e("#placeholderOption").html(b);e("#tabs").tabs();try{s("../../Experimental/jquery.Flot."+
a+".js")}catch(f){console.log(f)}e("#placeholderManual").html(n(e.plot.JUMExample.docu[a]));e(".jumflothelp").on("mouseover",q);e(".jumflothelp").on("mouseleave",function(){e("#FLOTtooltip").remove()})};e.plot.JUMExample.showDocuHelp=q;e.plot.JUMExample.getDocu=function(a,c,g){a=a.getOptions();c=(a.series[c]||a.grid[c]||a[c]).debug.createDocuTemplate();v=c.data;w=c.form;p=g;e(p).html(w)};e.plot.JUMExample.setDocu=function(a,c){var e="actDocu.data",b;b=a.split(".");e="actDocu.data";c=c.replace(/\r\n|\r|\n/g,
"<br>");for(var d=0;d<b.length;d++)0<b[d].length&&(e+='["'+b[d]+'"]');eval(e+("='"+c+"'"))};e.plot.JUMExample.createJSON=function(){var a=JSON.stringify(v),a=a.replace(/{/g,"{\n"),a=a.replace(/}/g,"}\n");e(p).html('<textarea rows="12" cols="80">'+a+"</textarea>")};e.plot.JUMExample.showSource=s;e.plot.JUMExample.docuObjectToTemplate=function(a,c){function g(a,b){var c="";if("object"===typeof a){var c='{"docu":""',d;for(d in a)if("object"===typeof a[d])"image"!==d?(c=e.isNumeric(d)?c+('\n,"'+d+'":'):
c+("\n,"+d+":"),c+=g(a[d],b),c+="}"):c+="\n,"+d+':{"docu":""}';else if("function"===typeof a[d])var f=a[d].toSource(),f=f.substr(8,f.indexOf(")")-7),c=!0===b?c+('\n,"'+d+'":{"docu":""}'):c+('\n,"'+d+'":{"docu":"",defVal:"'+f+'"}');else c=!0===b?c+('\n,"'+d+'":{"docu":"","defVal":"'+a[d]+'"}'):c+('\n,"'+d+'":{"docu":""}')}else c+='{"docu":""';return c}var b,d,f,h;b='z = {"docu":""\n'+(","+c+':{"docu":""\n');d=eval(b+"\n}\n}");for(var k=0;k<a.length;k++){f=b;var l=a[k].name.split(".");f+=",";for(h=
0;h<l.length-1;h++)f+=l[h]+':{"docu":"",';f+=l[l.length-1]+":";h=a[k].takeDefault?a[k].takeDefault:!1;f+=g(a[k].tree,h)+"}";for(h=0;h<l.length-1;h++)f+="}";f+="\n}\n}";e.extend(!0,d,eval(f))}return d};e.plot.JUMExample.extendDocuObject=function(a,c){function g(a,b){for(var c in a)"object"===typeof a[c]?b[c]&&g(a[c],b[c]):"docu"===c?b.docu&&0<b.docu.length&&(a.docu=b.docu):"defVal"===c&&(b.defVal&&0<b.defVal.length)&&(a.defVal=b.defVal)}var b=e.plot.JUMExample.docu[c],d=a[c].data,f;for(f in d)"0"!==
f&&("docu"!==f&&"defVal"!==f)&&delete d[f];b&&g(a,b);e.extend(!0,a,b)};e.plot.JUMExample.docuObjectToEdit=function(a,c){function e(a,c){b="<ul>";b+="<li>"+c.substr(c.lastIndexOf(".")+1);a.defVal&&(b+=" <i>default:"+a.defVal+"</i>");b+="<br>";b+='<textarea cols="50" rows="3" onblur="'+("$.plot.JUMExample.setDocu('"+c+".docu',this.value);")+'" >'+a.docu+"</textarea><br>";for(var h in a)"object"===typeof a[h]&&(b+=e(a[h],c+"."+h));return b+="</ul>"}var b='<form name="editObject">',b=b+'<input type="button" value="Create json" onclick="$.plot.JUMExample.createJSON();">',
b=b+e(a,c);return b+="</form>"};e.plot.JUMExample.docuObjectToHTML=n;e.plot.JUMExample.objectToDocumentation=t;e.plot.JUMExample.objectStringifyHTML=u;e.plot.JUMExample.getAPIHelp=r;e.plot.JUMExample.docu={}})(jQuery);$.plot.JUMExample.docu.background={docu:"Documentation",background:{docu:"<strong>Plugin to fraw background and/or overlay</strong>.<br>Background is drawn on an additional canvas context. Critical point is the use of zIndex, since some plugins change zIndex to negativ values.<br>See option zIndex how to handle this.",options:{docu:"",grid:{docu:"Standard grid object from Flot",background:{docu:"special options for background only",active:{docu:"activate the plugin",defVal:"false"},mode:{docu:"optional values are image, color and userdefined",
...
setZIndex:{docu:"option for setting all canvas to a specific value. Very helpful for using jQuery UI.<br>True sets background to 0, drawing area to 1 and highlight to 2<br>A number sets drawing area to the given number, background to number-- and highlight to number++ ",defVal:"false"}},overlay:{docu:"Specific options for drawing overlays. Overlay image is drawn on drawing area with given opacity.<br>This does not work for those plugins, that use hook draw",active:{docu:"Activates drawing an overlay",
defVal:"false"},image:{docu:"This is set to an image object, works similiar to image in background part"},opacity:{docu:"Opacity for drawing overlay image",defVal:"0.2"}}}}}};$.plot.JUMExample.docu.bandwidth={docu:"",defVal:"",bandwidth:{docu:"<strong>Plugin to create bandwidth charts</strong>",defVal:"none",data:{docu:"Data Array specific for Bandwidth chart",defVal:"none",0:{docu:"first data entry",defVal:"none",0:{docu:"Y-value",defVal:"none"},1:{docu:"X High-value",defVal:"none"},2:{docu:"X Low-value",defVal:"none"}},1:{docu:"more entries",defVal:"none"}},options:{docu:"options for bandwidth",defVal:"none",series:{docu:"series options for bandwidth",defVal:"none",
...
defVal:" drawEditDefault(octx,x,y,serie)"},drawHover:{docu:"function to draw hover shadow. It is defined in jquery.flot.mouse plugin.",defVal:" drawHoverDefault(octx,serie,dataIndex)"}}}}}};$.plot.JUMExample.docu.pyramids={docu:"Documentation",pyramid:{docu:"Plugin to create pyramids charts",data:{docu:"Data Array specific for pyramids chart",0:{docu:"first data entry",value:{docu:"Value(size) for data slice"},label:{docu:"Label of the data clice"}}},options:{docu:"options for pyramids",series:{docu:"series options for pyramids",pyramids:{docu:"pyramids only options",active:{docu:"activate the plugin",defVal:"false"},show:{docu:"how specific serie. this needs to be overwritten in data",
defVal:"false"},mode:{docu:"Decribes how a slice is shown. Actually pyramid and slice are supported",defVal:"pyramid"},fill:{docu:"Switches Fillmode for drawing of a slice",defVal:"true"},highlight:{docu:"Used to highlight in case of HOVER",opacity:{docu:"only Opacity is supported for Highlighting (yet)",defVal:"0.5"}},label:{docu:"description whether and how a label should be drawn",show:{docu:"Swichtes label drawing on or off",defVal:"false"},align:{docu:"Position of label",defVal:"center"},font:{docu:"Used font for Label",
...
highlight:{docu:"Describes how highlighting (in case of HOVER) is displayed",opacity:{docu:"Default for highlighting is to change opacity only",defVal:"0.5"}}}}}}};$.plot.JUMExample.docu.rectangle={docu:"Documentation",rectangle:{docu:"Plugin to create rectangle charts",data:{docu:"Data Array specific for rectangle chart",0:{docu:"first data entry",label:{docu:"standard label"},data:{docu:"standard in flot"},pos:{docu:"plugin internal cache for later support of hover etc",x:{docu:"x-position"},y:{docu:"y-position"},w:{docu:"width"},h:{docu:"height"},color:{docu:"guess yourself, what this could be :-)"}}}},options:{docu:"options for rectangle",series:{docu:"series options for rectangle",
